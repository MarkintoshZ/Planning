...
class PreActionState(State):
    def run(self):
        program = guidance(
            ...
            - terminate
            Choose an action by outputing the action name followed by the action arguments. Please terminate if the answer is complete, even if plan is not completed. 
            {{~/user}}
            {{#assistant~}}
